...
export default function SearchBox(placeholder = "What are you looking for?", openMenu, query) {
  ...
  const handleSearch = () => {
    if (localStorage && searchQuery !== "") {
      localStorage.setItem("vibesearch-history", localStorage.getItem("vibesearch-history") + "," + searchQuery);
    }
    ...
    <>
      {query !== 'wishlist' ? (
        <div className={`${styles.searchBox}`}>
          <div className={`${styles.searchBox__leftContainer}`} style={{ background: openMenu ? "rgba(255,255,255,.5)" : "#E7E7E7" }}>
            <Link
              onClick={() => {
                if (localStorage && searchQuery !== "") {
                  localStorage.setItem("vibesearch-history", localStorage.getItem("vibesearch-history") + "," + searchQuery);
            ...
            <input
              ...
              style={{ textOverflow: "ellipsis", opacity: openMenu ? 0 : 1 }}
            />
          </div>
          <div className={`${styles.searchBox__rightContainer}`} style={{ opacity: openMenu ? 0.7 : 1 }}>
            <input
              type="file"
              accept="image/*"
