...
export default function SearchResults() {
    ...
    useEffect(() => {
        if (accessData && accessData.data && accessData.data.session) {
            if (query === 'wishlist') {
                ...
                let input1 = query === "emptyEntry" && secondQuery ? secondQuery : query;
                let input2 = secondQuery || "";
                services.vibeIt(input1, input2, `${currentPage}`, "10", setSearchResults, accessData.data.session.access_token, searchResults).then(() => {
                    if (!secondQuery && localStorage) localStorage.removeItem('image-file');
                    setDatafetched(true);
                });
            }
        }
    ...
    useEffect(() => {
        ...
        keys.forEach(key => {
            ...
            if (product && product.wishlist_flag === 1) set.add(key);
            if (query === 'wishlist') set.add(key);
        });
    ...
    return (
        <div className={`${styles.SearchResults__ParentContainer}`}>
            ...
            <div className={`${styles.SearchResults__container}`} >
                <div className={`${styles.SearchResults}`} >
                    ...
                    <div className={`${styles.SearchResults__searchBox}`}>
                        {/* {Utilities.SearchBox(query !== "emptyEntry" ? query : "What are you looking for?", openMenu, query)} */}
                    </div>
                    <div className={`${styles.SearchResults__wrapper}`} onScroll={(event) => {
                        ...
                        {dataFetched && accessData && accessData.data && accessData.data.session && !searchResults.message
                            ? Utilities.SearchResultsElements(searchResults, setShowDetails, setProductDetails, accessData.data.session.access_token, addThisIndex, elementsInWishList, query, removeThisIndex)
                            : (dataFetched && searchResults.message
                                ? <div className={`${styles.SearchResults__loading}`}>No more relevant products</div>
                                : loading()
                            )
                        }
                        {dataFetched && searchResults.length === 0 && <div className={`${styles.SearchResults__loading}`}>{wishlistLoading()}</div>}
                        <div className={`montserrat ${styles.nextButtons}`}>
                            <div style={{ display: showNextPage && dataFetched && currentPage > 1 ? "inherit" : "none", marginRight: "12px" }}
                                className={`montserrat ${styles.nextButton}`}
                                onClick={() => { setShowNextPage(false); setCurrentPage(currentPage - 1); }}>
                                Prev Page
                            </div>
                            <div style={{ display: showNextPage && dataFetched ? "inherit" : "none" }}
                                className={`montserrat ${styles.nextButton}`}
                                onClick={() => { setShowNextPage(false); setCurrentPage(currentPage + 1); }}>
                                Next Page
                            </div>
            ...
            <div>
                {accessData && accessData.data && accessData.data.session && Components.ProductPage(query || "", productDetails.image, productDetails.product_title, productDetails.price, productDetails.product_url, setShowDetails, productDetails.id, accessData.data.session.access_token, showDetails, productDetails.description, productDetails.brand, productDetails.price_availaible, productDetails.additional_description, productDetails.additional_images)}
            </div>
        </div>
    );
}